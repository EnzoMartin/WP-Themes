...
<main class="main <?php echo roots_main_class(); ?>" role="main">
    <div class="main-bg">
        <?php get_template_part('templates/header', 'voice'); ?>
        <h2><?php echo get_option('purepings_contentdescription'); ?>
            <small class="pull-right"><?php echo get_post_meta($post->ID, 'voice_date', true); ?></small>
        </h2>
        <?php get_template_part('templates/content', 'page'); ?>
        <?php
        $faqs = get_post_meta($post->ID, 'voice_faqs', false);
        if ( !empty($faqs) ) {
            $leftColumn = [];
            $rightColumn = [];
            $i = 0;
        ...
        $voiceInfo = get_post_meta($post->ID, 'voice_textarea_info', true);
        if($voiceInfo){
            $voiceInfo = apply_filters( 'the_content', $voiceInfo );
            $voiceInfo = str_replace( ']]>', ']]&gt;', $voiceInfo );
        ?>
            <h2><?php echo get_option('purepings_contentadditional'); ?></h2>
            <div class="content"><?php echo $voiceInfo ?></div>
        <?php } ?>
    </div>
</main>