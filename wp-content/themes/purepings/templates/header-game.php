...
$imageMeta = get_post_meta($post->ID, 'game_image', true);
if($imageMeta){
    $image = wp_get_attachment_url($imageMeta, 'fullsize');
}

$headingMeta = get_post_meta($post->ID, 'game_text_heading', true);
if($headingMeta == ''){
    $headingMeta = get_the_title();
}
$subheadingMeta = get_post_meta($post->ID, 'game_text_subheading', true);
$urlMeta = get_post_meta($post->ID, 'game_text_url', true);
?>
