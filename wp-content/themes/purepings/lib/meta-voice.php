<?php
// Add the Meta Box
function voice_meta_box() {
    add_meta_box(
        'voice_meta_box', // $id
        'Voice Server Details', // $title
        'show_voice_meta_box', // $callback
        'voice', // $page
        'normal', // $context
        'high'); // $priority
}
add_action('add_meta_boxes', 'voice_meta_box');
...
$voice_meta_fields = array(
    ...
    array(
        'label'  => 'Icon',
        'desc'  => 'Small icon to show on the voice Server list page (16x16 px)',
        'id'    => $prefix.'icon',
        'type'  => 'image'
    ),
    array(
    ...
    array(
        'label' => 'Release Date',
        'desc'  => 'voice release date ex: Released on 01/20/12',
        'id'    => $prefix.'date',
        'type'  => 'date'
    ),
    array(
...
// The Callback
function show_voice_meta_box() {
    global $voice_meta_fields, $post;
    // Use nonce for verification
    echo '<input type="hidden" name="voice_meta_box_nonce" value="'.wp_create_nonce(basename(__FILE__)).'" />';

    // Begin the field table and loop
    echo '<table class="form-table">';
    foreach ($voice_meta_fields as $field) {
...
// Save the Data
function save_voice_meta($post_id) {
    global $voice_meta_fields;

    // verify nonce
    if (!isset($_POST['voice_meta_box_nonce']) || !wp_verify_nonce($_POST['voice_meta_box_nonce'], basename(__FILE__)))
        return $post_id;
    // check autosave
    ...
    foreach ($voice_meta_fields as $field) {
        ...
        if ($field['id'] == 'voice_locations' || $field['id'] == 'voice_features' || $field['id'] == 'voice_faqs') {
...
add_action('save_post', 'save_voice_meta');